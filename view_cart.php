	...
	<nav class="navbar navbar-expand-sm navbar-dark bg-dark" >
		...
		<li class="nav-item" ><a href="#bottom" class="nav-link">Contact</a></li>
			...
			<ul class="navbar-nav ml-auto " style="color: blue; font-size: 20px; float: left;">
			 <li class="nav-item" ><a href="product.php" class="nav-link">Back </a></li>
			 <li class="nav-item" ><a href="confirm_order.php" class="nav-link">Confirm order</a></li>
		</ul>
			</nav>
			  ...
			  $submit ="SELECT pro_id , how_many, cart_id FROM add_to_cart where buyer_id = $buyer_id ";
				...
				while($him=mysqli_fetch_array ($y) )
				{
					...
					while($row=mysqli_fetch_array ($x) ){
						...
						$total = $total + $him['how_many'];
						$sum = $sum + ($row['pro_price'] * $him['how_many']);
								
		}} ?>
						<div class="col-sm-12">
							<h1 style="color: red; font-size: 30px; text-align: left;">Total items : <?php echo $total;?> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Your bill : <?php echo $sum;?></h1>
				...
				<button style="border-style: double; border-color: gray; background-color: black; color: gray; height: 550px;"><h1 style="text-align: center;"> <?php echo $row['pro_name'];?></h1> 
					<?php echo '<img src="data:image/jpeg;base64,'.base64_encode($row['pro_pic']) .'" style="width:300px; height:200px;"/>'?><br><h1 style="font-size: 20px;">About: <?php echo $row['pro_discrip'];?></h1><br> <h1 style="font-size: 20px;">How many? <?php echo $him['how_many'];?><br><br>Rs. <?php echo $row['pro_price'];?></h1><br><a href="remove_to_cart.php?cart_id=<?php echo $him['cart_id'];?>">Remove from cart</a></button>
			</div>
